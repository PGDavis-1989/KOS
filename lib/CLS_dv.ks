...
//Creates circulization node at periapsis by default. Apoapsis if parameter is "apo"
Function createCircNode {
	parameter apo.
	if apo = "apo" {
		local nd is node(time:seconds + eta:apoapsis, 0, 0, circulariseDV_Apoapsis()).
		add nd.
	}
	else {
		local nd is node(time:seconds + eta:apoapsis, 0, 0, circulariseDV_Periapsis()).
		add nd.
	}
}
